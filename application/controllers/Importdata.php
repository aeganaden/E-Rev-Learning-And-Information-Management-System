...
//            if ($userInfo->password == sha1($this->input->post('password'))) {
    ...
    public function uploadfile() {
        ...
        if (!empty($this->session->userdata('insertion_info')['logged_in']) && $this->session->userdata('insertion_info')['logged_in'] == 1) {
            ...
            $col = "sch.schedule_id as sched_id, CONCAT(FROM_UNIXTIME(sch.schedule_start_time, '(%a %h:%i%p'),'-' ,FROM_UNIXTIME(sch.schedule_end_time, '%h:%i%p)')) as date_time, UCASE(CONCAT(lec.firstname, ' ', lec.midname, ' ', lec.lastname)) as fullname, lec.id_number as lec_id";
            $join = array(
                    ...
                    if($this->upload->do_upload("userfile")){
                        ...
                        for($i=1; $i<=count($sheetData);$i++){
                            if($i != 1){
                                ...
                                if($check1 && $check2 && $check3 && $check4){ //convert to shorter
                                    if(!is_numeric($valA = $sheetData[$i]['A']) || strlen($valA) != 9 || strcmp($lect_id, strval($valA)) != 0){   //9 exact digits of empid
