...
class Xlsx extends BaseReader {
    ...
    public function listWorksheetInfo($pFilename) {
        ...
        $rels = simplexml_load_string(
                //~ http://schemas.openxmlformats.org/package/2006/relationships"
                $this->securityScan($this->getFromZipArchive($zip, '_rels/.rels')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
        );
        foreach ($rels->Relationship as $rel) {
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml->xml(
                                ...
                                ), null, Settings::getLibXmlLoaderOptions()
                        );
    ...
    public function load($pFilename) {
        ...
        $wbRels = simplexml_load_string(
                //~ http://schemas.openxmlformats.org/package/2006/relationships"
                $this->securityScan($this->getFromZipArchive($zip, 'xl/_rels/workbook.xml.rels')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
        );
            ...
            switch ($rel['Type']) {
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme':
                    ...
                    $xmlTheme = simplexml_load_string(
                            $this->securityScan($this->getFromZipArchive($zip, "xl/{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                    );
            ...
            switch ($rel['Type']) {
                case 'http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties':
                    $xmlCore = simplexml_load_string(
                            $this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                    );
                ...
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    ...
                    $relsWorkbook = simplexml_load_string(
                            //~ http://schemas.openxmlformats.org/package/2006/relationships"
                            $this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . basename($rel['Target']) . '.rels')), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                    );
                    $relsWorkbook->registerXPathNamespace('rel', 'http://schemas.openxmlformats.org/package/2006/relationships');
                    ...
                    $xmlStrings = simplexml_load_string(
                            //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                            $this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                    );
                    ...
                    if ($xmlWorkbook->sheets) {
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            $xmlSheet = simplexml_load_string(
                                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                                    $this->securityScan($this->getFromZipArchive($zip, "$dir/$fileWorksheet")), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                            );
                                    ...
                                    foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                        ...
                                        if ($filterColumn->top10) {
                                            ...
                                            foreach ($filterColumn->top10 as $filterRule) {
                                                $column->createRule()->setRule(
                                                                (((isset($filterRule['percent'])) && ($filterRule['percent'] == 1)) ? Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT : Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE
                                                                ), (string) $filterRule['val'], (((isset($filterRule['top'])) && ($filterRule['top'] == 1)) ? Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP : Column\Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM
                                                                )
                                                        )
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    ...
                                    $commentsFile = simplexml_load_string(
                                            $this->securityScan($this->getFromZipArchive($zip, $relPath)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                                    );
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            $vmlDrawing = simplexml_load_string(
                                                    $this->securityScan($this->getFromZipArchive($zip, $vmlRelationship)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                                            );
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        $xmlDrawing = simplexml_load_string(
                                                $this->securityScan($this->getFromZipArchive($zip, $fileDrawing)), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
                                                ...
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing->setPath(
                                                            ...
                                                            $images[(string) self::getArrayItem(
                                                                    $blip->attributes('http://schemas.openxmlformats.org/officeDocument/2006/relationships'), 'embed'
                                                            )], false
                                                    );
    ...
    private function readRibbon(Spreadsheet $excel, $customUITarget, $zip) {
        ...
        if ($dataRels) {
            ...
            $UIRels = simplexml_load_string(
                    $this->securityScan($dataRels), 'SimpleXMLElement', Settings::getLibXmlLoaderOptions()
            );
            if ($UIRels) {
